...
demoExcelFile <- "cellstyles4.xlsx"
...
#test ----------------------------------------------------------------------
hp2 <- createCellStyle(wb, name = "hp2")
setFillPattern(hp2, fill = XLC$"FILL.SOLID_FOREGROUND")
setFillForegroundColor(hp2, color = XLC$"COLOR.GREEN")
rowIndex <- which(mtcars$hp > 200)
rowIndex <- rowIndex + 4
colIndex <- which(names(mtcars) == "hp") + 2
setCellStyle(wb, sheet = "mtcars", row = rowIndex, col = colIndex, 
             cellstyle = hp2)
